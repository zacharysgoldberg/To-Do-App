...
async def forgot_password(request: Request, email: str = Form(...), 
    ...
    if validate_email:
        ...
        <div style="width:100%;font-family: monospace;">
            ...
            <a href="http://127.0.0.1:8000/auth/reset-password/?reset_token={1:}" style="box-sizing:border-box;border-color:#1f8feb;border:solid 1px #1f8feb;border-radius:4px;color:#ffffff;font-size:16px;font-weight:bold;margin:0;padding:12px text-transform:capitalize">Reset Your Password</a>
            <p>If you did not request this, you can ignore this email.</p>
            <p>Your password will not change until you access the link above and create a new one.</p>
        </div>
        </body>
        </html>
        """.format(email, reset_token)
...
async def reset_password(request: Request, reset_token: str, new_password: str = Form(...), 
    ...
    user_data = db.query(Users).filter(Users.reset_token == reset_token).first()
